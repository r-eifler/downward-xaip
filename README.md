...
## Relax MUGS Computation

To compute all relaxed MUGS, the following configurations are available

### Branch-and-Bound with MSGS propagation

    --search 'relaxation_iterated([gsastar(evals=[blind], eval=ngs(hmax()), verbosity=silent)], propagate_msgs=true)

###  Iterative Search Space Extension

    --search 'astar_relaxations(evals=[blind], eval=ngs(hmax()))'
